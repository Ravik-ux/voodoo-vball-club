...
if 'view_mode' not in st.session_state:
    st.session_state.view_mode = "Input"  # Default view
...
# --- NAVIGATION LOGIC ---
# This creates the "Switch" at the bottom
with st.container():
    st.markdown('<div class="floating-nav">', unsafe_allow_html=True)
    if st.button(f"GO TO {'SCOREBOARD' if st.session_state.view_mode == 'Input' else 'INPUT PAGE'}"):
...
if st.session_state.view_mode == "Input":
    ...
    st.info("Tap the button at the bottom to see the full Colorful Scoreboard.")

else:
    st.title("📊 Official Live Scoreboard")
    active_set = st.radio("Viewing Stats for:", ["Set 1", "Set 2", "Set 3", "Match Totals"], horizontal=True)
    
    if active_set == "Match Totals":
        display_df = st.session_state.set_dfs["Set 1"] + st.session_state.set_dfs["Set 2"] + st.session_state.set_dfs["Set 3"]
    else:
        display_df = st.session_state.set_dfs[active_set]
    
    # The Colorful Table (Read-Only Page)
    st.table(apply_voodoo_style(display_df))
    st.success("Viewing Mode: High Contrast")
